...
function rotateMatrix(matrix) {
  ...
  while (z < matrix.length) {
    for (let i = nMatrix.length - 1; i >= 0; i -= 1) {
      arr[row][col] = nMatrix[i][z];
      col += 1;
    }
    row += 1;
    col = 0;
  ...
  for (let i = 0; i < nMatrix.length; i += 1) {
    for (let j = 0; j < nMatrix[0].length; j += 1) {
      nMatrix[i][j] = arr[i][j];
    }
  }
  return nMatrix;
...
function shuffleChar(str, iterations) {
  ...
  for (let i = 0; i < iterations; i += 1) {
    for (let j = 0; j < s.length; j += 1) {
      if (j % 2 === 0) {
        sStr += s.substring(j, j + 1);
      } else {
        rStr += s.substring(j, j + 1);
      }
    }
    s = sStr + rStr;
    sStr = '';
    rStr = '';
    if (str === s && iterations - i > i) {
      i = iterations - (iterations % (i + 1)) - 1;
    }
  }
  return s;
}
